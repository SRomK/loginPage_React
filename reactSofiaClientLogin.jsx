...
import LoadingButton from 'components/Material/LoadingButton';
...
let JQUERY = true; // If we're in jquery we send a custom event when the login was succesful, otherwise we redirect
const URL_AZURE_LAT = "https://login.microsoftonline.com/72e15514-5be9-46a8-8b0b-af9b1b77b3b8/oauth2/v2.0/authorize?client_id=dd7fb18f-c64b-47bb-836d-af9a91d71f0b&response_type=id_token&redirect_uri=https://www.efa.training&scope=openid%20email&nonce=123456";
...
// console.log('[LOGIN] saveToken', res)
	...
	render() {
		const { email, emailError, password, passwordError, error, redirect, rememberMe, showPassword, openForgotPassword, buttonLoading } = this.state

		return (
			<div id='loginContainer' className={styles.container}>
				{redirect && <Redirect to='/home/' />}
				<div className={styles.login}>
					<div id='loginImage' className={styles.logo}></div>
					<div className={styles.subtitle}>{lang.fsms}</div>
					...
					<FormControl className={styles.form} >
						<TextField 
							...
							name='email' 
							type='email' 
							label={lang.usernameEmail}
							error={emailError}
							className={styles.loginUser} 
							variant='outlined'
							value={email} 
							onChange={this.handleChange} 
							onKeyUp={this.handleKeyUp} // Save on Enter
							/>
							...
							InputProps={{
								endAdornment: 
									...
									<div className={styles.viewPasswordIcon}>
										<span className={this.getEyeIconClassname()} onClick={this.handleClickShowPassword}/>
									</div>
								</InputAdornment>                                
							}}/>
						<label>
						...
						<Link 
							id='liveLink'
							href='https://live.private-radar.com' 
							target='_blank'
							style={{ margin: 'auto', padding: '0.4em' }} >
							<span>Private-Radar Live</span>
						</Link>
						<Link 
							id='forgotMyPassword'
							href='#' 
				...
				{openForgotPassword && <ForgotPassword onClose={this.handleClose}/> }
			</div>
		)
	}
}
